...
export default async function Home() {
  ...
  const erc20Transfers = gnosisPayAccount && safeTokenData?.address
  ...
  const hasMoveFundsData =
    safeTokenData?.address &&
    user?.name &&
    user?.country &&
    currencyData?.code;

  const hasBalanceData =
    hasAtLeastOneActiveCard &&
    gnosisPayAccount &&
    currencyData?.code &&
    currencyData?.symbol &&
    safeTokenData?.address;
  ...
  const showConfigurationWarning =
    hasAtLeastOneActiveCard && gnosisPayAccount && safeTokenData?.address;
    ...
    <MainContent>
      ...
      >
        ...
        <div className="flex flex-row gap-4 sm:gap-0 justify-between items-start pb-5 sm:pb-0">
          ...
          {gnosisPayAccount && hasMoveFundsData && (
            <MoveFundsButton
              currencyName={currencyData?.code}
              account={gnosisPayAccount}
              tokenAddress={safeTokenData?.address}
