...
// Safe logo (mirrors the icon RainbowKit ships for its built-in Safe wallet).
const safeIcon =
...
/**
 * Custom RainbowKit Safe wallet.
 *
 * RainbowKit's built-in `safeWallet` calls wagmi's `safe()` connector with no
 * options, which drops the security configuration we relied on with AppKit.
 * This mirrors RainbowKit's Safe wallet but restores the original connector
 * options: only auto-connect when embedded within the official Safe interfaces.
 */
export const safeWallet = (): Wallet => ({
  ...
  iconAccent: "#12ff80",
  iconBackground: "#fff",
  iconUrl: async () => safeIcon,
  // Only surfaced when running inside an iframe (i.e. as a Safe App).
